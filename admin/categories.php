...
if(isset($_POST)&& !empty($_POST)){
 ...
 $squlform ="SELECT * FROM categories WHERE category = '$category' AND parent = '$post_parent'" ;
 if(isset($_GET['edit'])){
 $id = $edit_category['id'];
 $squlform = "SELECT * FROM categories WHERE category = '$category' AND parent = '$post_parent' AND id !='$id'" ;
  }
 $fresult = $db->query($squlform);
...
<!-- Category Table -->
  ...
  <div class="col-md-6">
    <table class="table table-bordered">
      <thead>
        <th></th>Category<th>Parent</th><th></th>
      </thead>
      <tbody>
