...
if (isset($_GET['add'])) {
  ...
  if ($_POST) {
   ...
   $emailCount = mysqli_num_rows($emailQuery);
    if ($emailCount != 0 ) {
      $errors[] = 'That email already used.';
    }
  ...
  foreach($required as $f){
    ...
    if (strlen($password) < 6) {
      $errors [] = 'You password must be at least 6 charecters.';
    }
     if ($password != $confirm) {
       $errors [] = 'Your password does not match the confirm.';
     }
...
.add-user {
  ...
  <h2 class="text-center">Add A New User</h2><hr>
   <form  action="users.php?add=1" method="post" class="add-user">
     <div class="form-groub col-md-5">
     ...
     <div class="form-groub col-md-10 text-right" style="margin-top: 30px">
